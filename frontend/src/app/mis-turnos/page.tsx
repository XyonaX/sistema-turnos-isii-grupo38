...
export default function MisTurnosPage() {
  ...
  const handleCancelar = async (id: string) => {
    try {
      ...
      setError(err.response?.data?.message || 'No se pudo cancelar el turno. Intenta de nuevo.');
      console.error(err);
    }
  };
      ...
      <main className="flex-1 px-4 py-12 max-w-4xl mx-auto w-full">
        ...
        {loading ? (
          ...
          <div className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-12 text-center">
            <svg className="w-16 h-16 text-[var(--text-muted)] mx-auto mb-4 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
            </svg>
            <h3 className="text-lg font-semibold text-[var(--text-primary)] mb-2">Sin turnos reservados</h3>
            <p className="text-[var(--text-muted)] mb-4">Todavía no tenés turnos. Comenzá a reservar en la sección de disponibilidad.</p>
            <a
              href="/disponibilidad"
              className="inline-block px-6 py-2 bg-gradient-to-r from-blue-500 to-cyan-500 text-white font-semibold rounded-lg hover:shadow-lg transition-shadow"
            >
